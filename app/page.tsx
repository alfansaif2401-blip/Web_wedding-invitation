...
// ================================================================
// § 1. DATA LAYER — All wedding details centralized
// ================================================================

const WEDDING = {
  bride: {
    fullName: 'Syarifah Intan Maughfiroh',
    nickname: 'Intan',
    father: 'Bapak Saifull',
    mother: 'Ibu Jinani',
  },
  groom: {
    fullName: 'Irfan Efendi',
    nickname: 'Irfan',
    father: 'Bapak M. Hisap',
    mother: 'Ibu Sariti',
  },
  event: {
    // !! IMPORTANT: time in local WIB (UTC+7)
    ...
    mapsUrl: 'https://maps.app.goo.gl/FYi4eWtmCoqMHcLg6',
  },
  gallery: [
...
type Phase = 'seal' | 'envelope' | 'rising' | 'blast' | 'content';
...
const CARD_ICONS: string[] = ['💍', '👑', '🕊️', '🌹', '🍰', '🥂'];

const JUNE_2026_FIRST_DOW = 1; // Monday (0=Sun, 1=Mon, …, 6=Sat)
...
// ── 4b. Memory match game ──────────────────────────────────────
function useMemoryGame() {
  const buildDeck = useCallback((): MemoryCard[] => {
  ...
  const flipCard = useCallback(
    (id: number) => {
      ...
      if (newSelected.length === 2) {
        ...
        const secondCard = card; // already the card with `id`
...
function OrnamentalDivider({ className = '' }: { className?: string }) {
  return (
    <div className={`flex items-center gap-3 w-full max-w-xs mx-auto my-5 ${className}`}>
      ...
      <span style={{ color: '#D4AF37', fontSize: 9, lineHeight: 1 }}>✦</span>
      <span style={{ color: '#D4AF37', fontSize: 14, lineHeight: 1 }}>✦</span>
      <div
        style={{
          flex: 1,
...
function DoilyLaceBorder() {
  return (
    ...
    >
      ...
      <g opacity="0.9">
        <path
          d="M0,0 L55,0 L55,4 L4,4 L4,55 L0,55 Z"
          fill="none"
          stroke="#D4AF37"
          strokeWidth="1.2"
        />
    ...
    >
      ...
      <motion.div
        ...
        style={{
          ...
          height: 210,
          background:
            'radial-gradient(circle, rgba(212,175,55,0.25) 0%, rgba(139,0,0,0.1) 50%, transparent 70%)',
        }}
...
// ================================================================
// § 7. ENVELOPE ANIMATION COMPONENT
// ================================================================
    ...
    >
      ...
      <motion.div
        style={{
          ...
          position: 'absolute',
          background:
            'linear-gradient(155deg, #FAF0D0 0%, #EDD99A 40%, #D4B878 100%)',
          borderRadius: 10,
          border: '2px solid rgba(212,175,55,0.55)',
          boxShadow:
            '0 24px 70px rgba(0,0,0,0.55), inset 0 1px 0 rgba(255,255,255,0.25)',
          transformStyle: 'preserve-3d',
          overflow: 'visible',
        }}
        ...
        animate={
          showEnvelope
            ? { scale: 1, opacity: 1, rotateX: 5 }
            : { scale: 0.75, opacity: 0, rotateX: 8 }
        }
      ...
      >
        ...
        >
          ...
          {/* Inner address line decorations */}
        ...
        <motion.div
          style={{
            ...
            height: 190,
            background:
              'linear-gradient(160deg, #FFFDF5 0%, #FDF5E0 50%, #F5E8C0 100%)',
            borderRadius: 6,
            border: '1px solid rgba(212,175,55,0.45)',
            boxShadow:
              '0 -6px 24px rgba(0,0,0,0.22), inset 0 1px 0 rgba(255,255,255,0.6)',
            zIndex: 4,
            display: 'flex',
          ...
          animate={
            phase === 'envelope'
              ...
              : phase === 'rising'
              ? { y: -148, scale: 1.06, opacity: 1 }
              : phase === 'blast'
              ? { y: -148, scale: 1.06, opacity: 0 }
              : { y: 0, scale: 1, opacity: 0 }
          }
          transition={
            phase === 'rising'
              ? { duration: 0.82, delay: 0.52, ease: [0.22, 1, 0.36, 1] }
              : phase === 'blast'
              ? { duration: 0.28 }
              : {}
          }
        >
          {/* Mini invite decoration */}
          <div
            style={{
              width: 30,
              height: 1,
      ...
      <motion.div
        ...
        style={{
          background:
            'radial-gradient(circle, #FAF0D0 0%, #D4AF37 40%, #FFE066 60%, transparent 80%)',
          width: 340,
          height: 230,
          zIndex: 20,
        }}
        ...
        animate={
          blasting
            ? { scale: [0, 2.2, 5], opacity: [0, 1, 0] }
            : { scale: 0, opacity: 0 }
        }
        transition={{ duration: 0.62, delay: 0.15, ease: 'easeOut' }}
        onAnimationComplete={() => {
          if (blasting) onBlastComplete();
        }}
      />
    </div>
  );
...
// ================================================================
// § 8. VINYL CONTROLLER COMPONENT
// ================================================================

function VinylController({
  playing,
  onToggle,
}: {
  playing: boolean;
    ...
    >
      ...
      <motion.div
        style={{
          ...
          width: 3,
          height: 26,
          background: 'linear-gradient(#FFE066, #9B7B1B)',
          borderRadius: 1.5,
          transformOrigin: 'top right',
          transition: 'transform 0.55s cubic-bezier(0.25,0.46,0.45,0.94)',
          transform: playing
            ? 'rotate(28deg) translateX(-6px) translateY(2px)'
            : 'rotate(8deg) translateX(-2px) translateY(1px)',
      ...
      {!playing && (
        <motion.div
          ...
          animate={{ opacity: 1 }}
        >
          <span style={{ color: '#D4AF37', fontSize: 13, fontWeight: 'bold', textShadow: '0 0 8px rgba(0,0,0,0.8)' }}>
            ▶
          </span>
        </motion.div>
      )}
...
// ================================================================
// § 9. CHRONO-MATRIX (JUNE 2026 CALENDAR)
// ================================================================

function ChronoMatrix() {
  const dayNames = ['Min', 'Sen', 'Sel', 'Rab', 'Kam', 'Jum', 'Sab'];

  // Build cells array: null for empty leading slots, then 1-30
  const cells: (number | null)[] = Array(JUNE_2026_FIRST_DOW).fill(null);
  for (let d = 1; d <= JUNE_2026_DAYS; d++) cells.push(d);

  // Sunday column indices (0) - in our grid, col 0 is Sunday
  const isSundayCol = (idx: number) => idx % 7 === 0;
  const isSaturdayCol = (idx: number) => idx % 7 === 6;
        ...
        {cells.map((day, i) => (
          <div key={i} className="flex items-center justify-center" style={{ height: 38 }}>
            {day !== null && (
              <>
                {day === 13 ? (
                  /* Wedding day — pulsing crimson/gold highlight */
                  <motion.div
                    ...
                    style={{
                      ...
                      borderRadius: '50%',
                      background:
                        'linear-gradient(135deg, #9B0010 0%, #C8102E 60%, #9B0010 100%)',
                  ...
                  >
                    ...
                    <motion.span
                      className="absolute"
                      style={{ top: -6, right: -5, color: '#FFE066', fontSize: 9 }}
                      animate={{ rotate: 360 }}
                  ...
                  <span
                    style={{
                      fontFamily: 'Cormorant Garamond, Georgia, serif',
                      color: isSundayCol(i) ? '#C8102E' : '#e8d5b0',
                      fontSize: 12,
                      opacity: 0.8,
                    }}
                  >
      ...
      <div className="flex flex-col items-center gap-2">
        <motion.div
          ...
          style={{
            ...
            height: 70,
            background:
              'linear-gradient(145deg, rgba(139,0,0,0.28) 0%, rgba(212,175,55,0.08) 100%)',
            border: '1px solid rgba(212,175,55,0.4)',
            borderRadius: 10,
            boxShadow:
              'inset 0 1px 0 rgba(255,255,255,0.04), 0 4px 16px rgba(0,0,0,0.35)',
          }}
        >
          {/* Top sheen */}
          <div
...
// ================================================================
// § 11. MEMORY MATCH GAME COMPONENT
// ================================================================
  ...
  return (
    <div className="flex flex-col items-center gap-4 w-full">
      ...
      <div className="flex items-center justify-between w-full max-w-xs">
        ...
        >
          Langkah:{' '}
          <span style={{ color: '#FFE066', fontWeight: 700 }}>{moves}</span>
        </span>
        <span
          style={{
            fontFamily: 'Cormorant Garamond, Georgia, serif',
      ...
      <AnimatePresence>
        {won && (
          <motion.div
            ...
            style={{
              background:
                'linear-gradient(135deg, rgba(139,0,0,0.4) 0%, rgba(212,175,55,0.18) 100%)',
        ...
        {cards.map((card) => (
          <motion.div
            ...
            transition={{
              ...
              scale: card.matched
                ? { duration: 0.35, times: [0, 0.5, 1] }
                : { duration: 0.1 },
            }}
            onClick={() => flipCard(card.id)}
            whileHover={
              !card.flipped && !card.matched ? { scale: 1.05, y: -2 } : {}
            }
          >
            {/* Card Back */}
            <div
              style={{
                ...
                WebkitBackfaceVisibility: 'hidden',
                background:
                  'linear-gradient(145deg, #5A0010 0%, #8B0000 50%, #4A0008 100%)',
            ...
            >
              ...
              <span style={{ color: '#D4AF37', fontSize: 20, position: 'relative', zIndex: 1 }}>
                ✦
              </span>
            </div>
            ...
            <div
              style={{
                ...
                boxShadow: card.matched
                  ? '0 0 14px rgba(212,175,55,0.35), inset 0 0 10px rgba(212,175,55,0.08)'
...
function GalleryPhoto({
  ...
  return (
    <motion.div
      ...
      style={{
        aspectRatio: '4 / 5',
        background: 'linear-gradient(145deg, #1E0D00, #2D1800)',
      ...
      whileHover={{
        scale: 1.025,
        boxShadow: '0 14px 40px rgba(0,0,0,0.6), 0 0 20px rgba(212,175,55,0.15)',
    ...
    >
      ...
      {!imgError ? (
        <img
          ...
          style={{
            ...
            height: '100%',
            objectFit: 'cover',
            filter: 'sepia(18%) contrast(1.04) brightness(0.93) saturate(1.1)',
            display: 'block',
          }}
      ...
      <div
        style={{
          ...
          inset: 0,
          background:
            'linear-gradient(to bottom, transparent 55%, rgba(10,5,0,0.65) 100%)',
          pointerEvents: 'none',
          zIndex: 5,
        }}
      />
    ...
    >
      ...
      <div
        style={{
          ...
          boxShadow:
            '0 0 0 1px rgba(212,175,55,0.12), 0 24px 70px rgba(0,0,0,0.75), inset 0 0 32px rgba(212,175,55,0.04)',
          position: 'relative',
        }}
      >
        {!imgError ? (
          <img
            ...
            style={{
              ...
              height: '100%',
              objectFit: 'cover',
              filter: 'sepia(12%) contrast(1.06) brightness(0.94)',
              display: 'block',
            }}
            onError={() => setImgError(true)}
          />
          ...
          <div
            style={{
              width: '100%',
              height: '100%',
              background: 'linear-gradient(160deg, #2d1500, #1a0800, #0d0400)',
              display: 'flex',
              flexDirection: 'column',
              alignItems: 'center',
              justifyContent: 'center',
        ...
        <div
          style={{
            ...
            inset: 0,
            background:
              'linear-gradient(to bottom, transparent 50%, rgba(10,5,0,0.7) 100%)',
            pointerEvents: 'none',
          }}
        />
      </div>

      {/* Doily border overlay */}
      <DoilyLaceBorder />
...
// ================================================================
// § 14. LOADING INTRO SCREEN (seal + envelope + blast)
// ================================================================
...
function IntroScreen({
  ...
  return (
    <motion.div
      ...
      style={{
        background:
          'radial-gradient(ellipse at 40% 60%, #1a0800 0%, #0a0500 60%, #050200 100%)',
      }}
    ...
    >
      ...
      <div
        style={{
          ...
          height: 3,
          background:
            'linear-gradient(to right, transparent, #D4AF37, #FFE066, #D4AF37, transparent)',
        }}
      />
...
function MainContent() {
  return (
    <motion.main
      key="main"
      className="relative max-w-2xl mx-auto px-4 pb-28"
      initial={{ opacity: 0 }}
      animate={{ opacity: 1 }}
    ...
    >
      ...
      <section className="relative flex flex-col items-center justify-center min-h-screen py-20 text-center overflow-hidden">
        ...
        <div className="absolute inset-0 pointer-events-none">
          <motion.div
            style={{
              ...
              borderRadius: '50%',
              background:
                'radial-gradient(circle, rgba(139,0,0,0.12) 0%, rgba(212,175,55,0.04) 50%, transparent 70%)',
            }}
        ...
        >
          ...
          {/* Ampersand divider */}
          <motion.div
            className="my-7 flex items-center justify-center gap-5"
      ...
      {/* ──────────── OPENING BLESSING ──────────── */}
      <motion.section
        className="section-card p-8 mb-8 text-center"
        initial={{ opacity: 0, y: 50 }}
      ...
      >
        ...
        >
          ...
          menyelenggarakan walimatul ursy putra-putri kami. Kami mengundang
          Bapak / Ibu / Saudara/i untuk hadir memberikan doa restu kepada kedua
          mempelai.
        </p>
      </motion.section>
      ...
      >
        ...
        <motion.div
          ...
          style={{
            background:
              'linear-gradient(135deg, rgba(139,0,0,0.25) 0%, rgba(212,175,55,0.1) 100%)',
        ...
        {/* Akad & Resepsi columns */}
        <div className="grid grid-cols-2 gap-5 mb-6">
          {[
            {
              title: 'Akad Nikah',
              time: WEDDING.event.akadTime,
              icon: '💍',
            },
            {
              title: 'Resepsi',
              time: WEDDING.event.resepsiTime,
              icon: '🥂',
            },
          ].map(({ title, time, icon }) => (
            <div
        ...
        <div
          ...
          style={{
            background:
              'linear-gradient(145deg, rgba(26,10,0,0.8), rgba(10,5,0,0.95))',
      ...
      {/* ──────────── CHRONO-MATRIX: CALENDAR ──────────── */}
      <motion.section
        className="section-card p-8 mb-8"
        initial={{ opacity: 0, y: 50 }}
        whileInView={{ opacity: 1, y: 0 }}
        viewport={{ once: true, margin: '-60px' }}
        transition={{ duration: 0.75 }}
      ...
      >
        ...
        >
          Temukan semua pasangan kartu — seperti dua jiwa yang berjodoh
          satu sama lain ✦
        </p>
        <div className="flex justify-center">
          <MemoryMatchGame />
      ...
      >
        ...
        <div
          ...
          style={{
            background:
              'linear-gradient(145deg, rgba(26,10,0,0.7), rgba(10,5,0,0.9))',
            border: '1px solid rgba(212,175,55,0.18)',
          }}
        >
          <p
          ...
          >
            &ldquo;Semoga Allah memberkati kalian berdua, memberkahi kalian
            melalui satu sama lain, dan menyatukan kalian dalam kebaikan.&rdquo;
          </p>
          <p
            style={{
              fontFamily: 'Cormorant Garamond, Georgia, serif',
...
export default function WeddingInvitationPage() {
  ...
  const startFadeIn = useCallback(() => {
    ...
    const steps = AUDIO_FADE_DURATION_MS / AUDIO_FADE_INTERVAL_MS; // 50
    const increment = 1.0 / steps; // 0.02 per step
  ...
  // ── Attach fallback listeners for autoplay policy bypass ────
  const attachFallbackListeners = useCallback(() => {
    ...
    const handler = () => {
      ...
      audio
        ...
        .catch(() => {/* silently ignore subsequent failures */});
  ...
  const handleOpenInvitation = useCallback(() => {
    // CRITICAL FIRST LINE: invoke play() inside this user-gesture handler
    // to satisfy browser autoplay security policy.
    try {
      const audio = audioRef.current;
      if (audio) {
        audio
          ...
          .catch((err: unknown) => {
            // DOMException: autoplay blocked — attach fallback
      ...
      // Safety net for any synchronous audio errors
      attachFallbackListeners();
    }

    // ── Kick off the 3D animation phase timeline ────────────────
    // Phase 0: 'envelope' — seal disappears, envelope appears
    setPhase('envelope');

    // Phase 1: 800ms — flap rotates, sheet rises
    setTimeout(() => setPhase('rising'), 300);

    // Phase 2: 1600ms — blast reveal starts
    setTimeout(() => setPhase('blast'), 1600);

    // Phase 3: 2200ms — main content appears (backup timer)
    setTimeout(() => setPhase('content'), 2400);
  }, [startFadeIn, attachFallbackListeners]);

  // ── Blast complete callback (fired by EnvelopeScene) ────────
  ...
  return (
    <div className="luxury-bg min-h-screen">
      {/* Global top/bottom gold borders */}
      <div
        style={{
          position: 'fixed',
          top: 0,
